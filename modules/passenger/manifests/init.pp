class passenger {
	exec { 
		"/usr/local/bin/gem install passenger -v=3.0.17":
			user => root,
			group => root,
			alias => "install_passenger",
			before => [File["passenger_conf"],Exec["passenger_apache_module"]],
			unless => "ls /usr/local/lib/ruby/gems/1.9.1/gems/passenger-3.0.17/"
	}
	exec {
		"sudo /usr/local/bin/passenger-install-apache2-module --auto":
			...
			unless => "ls /usr/local/lib/ruby/gems/1.9.1/gems/passenger-3.0.17/ext/apache2/mod_passenger.so"
	}
	file {
